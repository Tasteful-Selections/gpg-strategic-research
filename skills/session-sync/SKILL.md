...
2. **Detect recent sync** — `git log --oneline -1`. If the last commit is a `sync:` or `wrap:` commit from within the last 5 minutes, ask: "You just synced {N} minutes ago. Sync again?" Prevents accidental double-commits.
